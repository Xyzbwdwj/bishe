...
parser.add_argument('--epochs', default=50000, type=int, help='number of total epochs to run (default:50k)')
parser.add_argument('--batch-size', default=200, type=int,help='mini-batch size (default: 200)')
parser.add_argument('-a', '--adam', default=0, type=int, help='whether to use ADAM or not (default: 0)')
...
parser.add_argument('-g', '--gpu', default=1, type=int, help='whether enable GPU computing (default:1), if more than one GPU, number indicates GPU device')
parser.add_argument('--hidden-n', default=200, type=int, help='Hidden dimension size(default:200)')
...
parser.add_argument('--ae', default=0, type=int, help='whether target = input (default=0)')
parser.add_argument('--net',default='ConvRNN',type=str,help='Name of the network class (default: ConvRNN)')
parser.add_argument('--act', default='sigmoid', type=str, help='Activation of output (default: sigmoid)')
parser.add_argument('--rnn_act', default='', type=str, help='Activation of rnn units (default: tanh)')
parser.add_argument('--pred',default=0,type=int,help='Prediction horizon (default:0)')
parser.add_argument('--resume', default='', type=str, help='path to latest checkpoint (default: none)')
parser.add_argument('--Hregularized_l1',default=0,type=float,help='penalty for hidden unit firing (l1 regularized)')
parser.add_argument('--Hregularized',default=0,type=float,help='penalty for hidden unit firing (l2 regularized)')
...
def main():
    ...
    start = time.time()
    net, loss_list = train_minibatch(X, Y, h0, n_epochs, net, criterion, \
        optimizer, RecordEp, args.Hregularized)
...
def train_minibatch(X, Y, h0, n_epochs, net, criterion, optimizer, RecordEp, lamda):
    ...
    while stop == 0 and epoch < n_epochs:
        ...
        for batch in range(int(N/args.batch_size)):
            ...
            loss2 = lamda*criterion(h_t,torch.zeros(h_t.shape).to(X_mini.device)) + args.Hregularized_l1*criterion_l1(h_t,torch.zeros(h_t.shape).to(X_mini.device))
            loss = loss1 + loss2
