...
class ElmanRNN:
	...
	def forward_propagation(self,X,Target,h0):
		h = np.zeros((np.int(self.N),np.int(self.bptt_depth))); h[:,0] = h0
	...
	def bptt(self,X,Target,h0):
		...
		for t in np.arange(1,np.int(self.bptt_depth-1))[::-1]: # loop from the latest-1:1
			dLdo = delta[:,t]*y_hat[:,t]*(1-y_hat[:,t])
			dLdV[:,:,t] = np.matmul(dLdo.reshape(np.int(self.N),1), (h[:,t].reshape(1,np.int(self.N))))
			dLdh[:,:,t] = np.matmul(np.matmul(self.W.T,np.diag(1-np.power(h[:,t+1],2))),dLdh[:,:,t+1]) + np.matmul(self.V.T,dLdo.reshape(np.int(self.N),1))
			dLdW[:,:,t] = np.matmul(np.matmul(np.diag(1-np.power(h[:,t],2)),dLdh[:,:,t]),h[:,t-1].reshape(1,np.int(self.N)))
...
class ElmanRNN_pytorch_module(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        self.n_layers = 1
        self.rnn = nn.RNN(self.input_dim, self.hidden_dim, self.n_layers, batch_first=True, nonlinearity = 'tanh')   
...
class ElmanRNN(nn.Module):
    ...
    def forward(self, x, h0):
        ...
        ht = h0; z = torch.zeros((batch_size,SeqN,self.hidden_dim)).to(x.device)
        for t in range(SeqN):
...
class ElmanRNN_pred_feedback(nn.Module):
    ...
    def forward(self, x, h0):
        ...
        for t in range(SeqN-1):
            ...
            htp1 = self.tanh(self.input_linear(yt) + self.hidden_linear(ht)) # predict one-step ahead
            z[:,t+1,:] = htp1 # z_{t+1} = h_{t+1|t}
        out = self.act(self.linear3(z))
        return out, ht
...
class ElmanRNN_sparse(nn.Module):
    ...
    def forward(self, x, h0):
        batch_size, SeqN, _ = x.shape
        ht = h0; z = torch.zeros(x.shape).to(x.device)
        mask = torch.zeros(1,batch_size,self.hidden_dim).bernoulli_(1-self.dropout).to(x.device)
        for t in range(SeqN):
...
class RateRNN(nn.Module):
    ...
    def forward(self, x, h):
        ...
        for t in range(SeqN):
            ...
            h_ = (1-self.dt/self.tau)*h + \
                self.dt/self.tau*(self.linear_hh(self.act(h)) \
...
class RateRNN_v2(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim,tau,dt):
        ...
        self.linear_hh = nn.Linear(self.hidden_dim, self.hidden_dim,bias=None)
        self.act = nn.Sigmoid() #activation function
        self.act2 = nn.Softmax(2) #activation function, normalization across hidden units
    def forward(self, x, h):
        batch_size, SeqN, _ = x.shape
        # hidden = []
        hidden = torch.zeros((batch_size,SeqN,self.hidden_dim)).to(x.device)
    ...
    def forward(self, x, h):
        ...
        for t in range(SeqN):
            ...
            h_ = (1-self.dt/self.tau)*h + \
                ...
                + self.linear_ih(x[:,t:t+1,:]))  \
                + torch.randn(h.shape).to(x.device)*0.01
            h_tp1 = (1-self.dt/self.tau)*h_ + self.dt/self.tau*(self.linear_hh(self.act(h_)))  \
                + torch.randn(h.shape).to(x.device)*0.01
            h = h_
            hidden[:,t,:] = h_tp1[:,0,:]
        # hidden = torch.stack(hidden,1)
...
class RateRNN_dale(nn.Module):
    ...
    def forward(self, x, h):
        ...
        for t in range(SeqN):
            # hidden.append(h[0:1,0,:])
            h_ = (1-self.dt/self.tau)*h + \
                self.dt/self.tau*(torch.matmul(self.m(self.linear_hh(self.act(h))),mask) \
                + self.linear_ih(x[:,t:t+1,:]))  \
                + torch.randn(h.shape).to(x.device)*0.01
            h = h_
            hidden[:,t,:] = h[:,0,:]
...
class RateRNN_dale_v2(nn.Module):
    ...
    def forward(self, x, h):
        ...
        for t in range(SeqN):
            ...
            h_ = (1-self.dt/self.tau)*h + self.dt/self.tau*(I) \
                + torch.randn(h.shape).to(x.device)*0.01
            h = h_
            hidden[:,t,:] = h[:,0,:]
            # I_record[:,t,:] = I[:,0,:]
...
class ConvRNN(nn.Module):
    ...
    def __init__(self, kernel, hidden_dim, H_out, W_out):
        ...
        self.H_out = H_out
        self.W_out = W_out
        self.conv = nn.Conv2d(in_channels=1,out_channels=1,kernel_size=self.kernel,bias=False)
        self.deconv = nn.ConvTranspose2d(in_channels=1,out_channels=1,kernel_size=self.kernel,bias=False)
        self.act = nn.Sigmoid()
        self.rnn = nn.RNN(self.H_out*self.W_out, self.hidden_dim, 1, batch_first=True, nonlinearity = 'tanh')  
    ...
    def forward(self, x, h0):
        ...
        for t in range(T):
            out[:,:,t,:,:] = self.deconv(torch.reshape(z[:,t,:],(N,1,self.H_out,self.W_out)))
        return out, h 
...
class ConvRNN_tp1(nn.Module):
    ...
    *_tp1: Iterate one more step for future prediction
    ...
    def forward(self, x, h0):
        ...
        for t in range(T):
            x_conv = self.conv(x[:,:,t,:,:])
            ht = self.tanh(self.input_linear(torch.reshape(x_conv[:,0,:,:],(N,-1))) + self.hidden_linear(ht))
